...
static void log_msg(const char *fmt, ...) {
    if (!g_log) {
        ...
        char buf[512]; time_t tn = time(NULL); struct tm tmn; localtime_r(&tn, &tmn);
        snprintf(buf, sizeof(buf), "/var/log/%s", user); mkdir(buf, 0755);
        snprintf(buf, sizeof(buf), "/var/log/%s/vox-%04d%02d%02d-%02d%02d%02d.log",
                 user, tmn.tm_year+1900, tmn.tm_mon+1, tmn.tm_mday, tmn.tm_hour, tmn.tm_min, tmn.tm_sec);
    ...
    while (g_running) {
        if (g_toggle) {
            ...
            if (!recording) {
                ...
                /* Drain any spurious toggle that arrived during cold-start */
                g_toggle = 0;
