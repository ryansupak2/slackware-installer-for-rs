...
void Bar::renderTags()
{
	for (auto &tag : _tags) {
		...
		if (tag.state & TagState::Active) {
			auto &ac = colorActive.fg;
			cairo_set_source_rgba(_painter, ac.r/255.0, ac.g/255.0, ac.b/255.0, ac.a/255.0);
			cairo_rectangle(_painter, tag.component.x, 0, tag.component.width() + paddingX*2, _bufs->height);
		...
		for (auto ind = 0; ind < indicators; ind++) {
			auto w = ind == tag.focusedClient ? 5 : 5;
			cairo_rectangle(_painter, tag.component.x + 2, ind*6+2, w, w);
			cairo_fill(_painter);
		}
	}
}
