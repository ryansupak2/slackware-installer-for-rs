...
{
	Client *c;
	int n = 0, idx = 0; Client *sel = focustop(m);
	...
	if (n)
		snprintf(m->ltsymbol, LENGTH(m->ltsymbol), "[%d/%d]", idx, n);
	else
		snprintf(m->ltsymbol, LENGTH(m->ltsymbol), "[0]");
	if ((c = focustop(m)))
		wlr_scene_node_raise_to_top(&c->scene->node);
}
...
tagprev(const Arg *arg)
{
	...
	if (!sel || !sel->tags)
		return;
	for (tag = 1; tag < TAGMASK; tag <<= 1)
		if (sel->tags & tag) break;
	tag = (tag == 1) ? (1 << (TAGCOUNT - 1)) : (tag >> 1);
	sel->tags = tag;
	focusclient(focustop(selmon), 1);
	arrange(selmon);
	printstatus();
...
tagnext(const Arg *arg)
{
	...
	if (!sel || !sel->tags)
		return;
	for (tag = 1 << (TAGCOUNT - 1); tag; tag >>= 1)
		if (sel->tags & tag) break;
	tag = (tag == (1 << (TAGCOUNT - 1))) ? 1 : (tag << 1);
	sel->tags = tag;
	focusclient(focustop(selmon), 1);
	arrange(selmon);
	printstatus();
...
viewprev(const Arg *arg)
{
	...
	if (!selmon)
		return;
	for (tag = 1; tag < TAGMASK; tag <<= 1)
		if (selmon->tagset[selmon->seltags] & tag) break;
	tag = (tag == 1) ? (1 << (TAGCOUNT - 1)) : (tag >> 1);
	selmon->seltags ^= 1;
