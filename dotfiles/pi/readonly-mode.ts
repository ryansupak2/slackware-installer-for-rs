...
export default function readonlyMode(pi: ExtensionAPI): void {
  ...
  function toggleReadonly(ctx: ExtensionContext): void {
    ...
    if (enabled) {
      ...
      ctx.ui.notify("Read Only mode ENABLED (file writes blocked)", "warning");
    } else {
      pi.setActiveTools([...FULL_TOOLS]);
      ctx.ui.notify("Read Only mode DISABLED (file writes are reenabled)", "warning");
      injectOffMessage = true;
    }
  ...
  pi.on("before_agent_start", async () => {
    if (!enabled) {
      if (injectOffMessage) {
        injectOffMessage = false;
        return {
          message: {
            customType: "readonly-mode-context",
            content: "Read-only mode is now OFF. You may freely use edit, write, bash, and all file-modification tools.",
            display: false,
          },
        };
      }
