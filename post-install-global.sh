...
utilities=("help" "mediainfo" "midnight-commander" "wifi-manager" "net-watch" "htop" "tmux")
audio_dsp=("sox")
...
echo ""
log_msg INFO "Global setup complete. Run post-install-user.sh for per-user configs."
# The automatic final shared copy to /usr/local/share/slackware-installer-for-rs/
# has been removed per user request.
#
# rsync is NEVER auto-installed by this script. It will only ever be installed
# if the user explicitly adds "rsync" to a package install step they select
# (or runs "slackpkg install rsync" manually).
#
# If you want the installer repo copied to /usr/local/share for other users,
# run one of these manually after the script (or add a menu item for it):
#   rsync -av --exclude=setup.keys* /root/slackware-installer-for-rs/ /usr/local/share/slackware-installer-for-rs/
#   (or the cp -a equivalent if rsync is not present)
